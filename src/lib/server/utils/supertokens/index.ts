...
import type { Tokens, CodeDetails } from "$lib/server/utils/supertokens/cookieHelpers";
...
const SuperTokensHelpers = {
  ...
  async sendPasswordlessInvite(info: PasswordlessEmailInfo | PasswordlessPhoneInfo): Promise<CodeDetails> {
    ...
    const link = `${env.DOMAIN}${commonRoutes.loginPasswordless}?token=${code.linkCode}`;
...
function createReturnURLForProvider(providerId: string, returnUrl?: string | null): string {
  let url = `${env.DOMAIN}${commonRoutes.loginThirdParty}?provider=${providerId}`;
